...
pub fn system_directory() -> Option<std::path::PathBuf> {
    let callback = unsafe { ENVIRONMENT_CALLBACK? };
    let mut directory: *const libc::c_char = ptr::null();
    let ok = unsafe {
        callback(
            libretro_sys::ENVIRONMENT_GET_SYSTEM_DIRECTORY,
            &mut directory as *mut _ as *mut libc::c_void,
        )
    };
    if !ok || directory.is_null() {
        return None;
    }
...
impl<B: Core> Retro<B> {
    ...
    pub fn on_set_environment(callback: libretro_sys::EnvironmentFn) {
        ...
        let supports_no_game = true;
        unsafe {
            callback(
                libretro_sys::ENVIRONMENT_SET_SUPPORT_NO_GAME,
                &supports_no_game as *const _ as *mut libc::c_void,
            );
        }
    }
...
impl RuntimeHandle {
    pub fn is_keyboard_key_pressed(&mut self, keycode: u32) -> bool {
        unsafe { (self.input_state_callback)(0, libretro_sys::DEVICE_KEYBOARD, 0, keycode) != 0 }
    }
...
macro_rules! libretro_core {
    ($core: path) => {
        ...
        static mut VIDEO_REFRESH_CALLBACK: Option<$crate::libretro_sys::VideoRefreshFn> = None;
        static mut AUDIO_SAMPLE_CALLBACK: Option<$crate::libretro_sys::AudioSampleFn> = None;
        static mut AUDIO_SAMPLE_BATCH_CALLBACK: Option<$crate::libretro_sys::AudioSampleBatchFn> =
            None;
        static mut INPUT_POLL_CALLBACK: Option<$crate::libretro_sys::InputPollFn> = None;
        static mut INPUT_STATE_CALLBACK: Option<$crate::libretro_sys::InputStateFn> = None;
        ...
        pub unsafe extern "C" fn retro_init() {
            ...
            if let Some(callback) = VIDEO_REFRESH_CALLBACK {
                (&mut *LIBRETRO_INSTANCE).on_set_video_refresh(callback);
            }
            if let Some(callback) = AUDIO_SAMPLE_CALLBACK {
                (&mut *LIBRETRO_INSTANCE).on_set_audio_sample(callback);
            }
            if let Some(callback) = AUDIO_SAMPLE_BATCH_CALLBACK {
                (&mut *LIBRETRO_INSTANCE).on_set_audio_sample_batch(callback);
            }
            if let Some(callback) = INPUT_POLL_CALLBACK {
                (&mut *LIBRETRO_INSTANCE).on_set_input_poll(callback);
            }
            if let Some(callback) = INPUT_STATE_CALLBACK {
        ...
        pub unsafe extern "C" fn retro_set_video_refresh(
            ...
            VIDEO_REFRESH_CALLBACK = Some(callback);
            if LIBRETRO_INSTANCE != 0 as *mut _ {
                (&mut *LIBRETRO_INSTANCE).on_set_video_refresh(callback);
            }
        }
        ...
        pub unsafe extern "C" fn retro_set_audio_sample(
            ...
            AUDIO_SAMPLE_CALLBACK = Some(callback);
            if LIBRETRO_INSTANCE != 0 as *mut _ {
                (&mut *LIBRETRO_INSTANCE).on_set_audio_sample(callback);
            }
        }
        ...
        pub unsafe extern "C" fn retro_set_audio_sample_batch(
            ...
            AUDIO_SAMPLE_BATCH_CALLBACK = Some(callback);
            if LIBRETRO_INSTANCE != 0 as *mut _ {
                (&mut *LIBRETRO_INSTANCE).on_set_audio_sample_batch(callback);
            }
        }
        ...
        pub unsafe extern "C" fn retro_set_input_poll(callback: $crate::libretro_sys::InputPollFn) {
            INPUT_POLL_CALLBACK = Some(callback);
            if LIBRETRO_INSTANCE != 0 as *mut _ {
                (&mut *LIBRETRO_INSTANCE).on_set_input_poll(callback);
            }
        }
        ...
        pub unsafe extern "C" fn retro_set_input_state(
            ...
            INPUT_STATE_CALLBACK = Some(callback);
            if LIBRETRO_INSTANCE != 0 as *mut _ {
                (&mut *LIBRETRO_INSTANCE).on_set_input_state(callback);
            }
        }
